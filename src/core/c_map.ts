...
class c_map {
    ...
    IsBlocked(x: number, y: number, w: number, h: number, blocking: number): boolean {
        ...
        if(this.tmGet(this.XYG(x + w - 1), this.XYG(y + h - 1)) <= blocking){ return true;}

        //other cases to block like statics/buildings etc
        if(this.BGrid.hasOwnProperty(this.XYG(x) + "_" + this.XYG(y))){ return true; }
        if(this.BGrid.hasOwnProperty(this.XYG((x + w - 1)) + "_" + this.XYG(y))){ return true; }
        if(this.BGrid.hasOwnProperty(this.XYG(x) + "_" + this.XYG(y + h - 1))){ return true; }
        if(this.BGrid.hasOwnProperty(this.XYG(x + w - 1) + "_" + this.XYG(y + h - 1))){ return true; }
    ...
    MoveVec(d: any, dt: number, _block: number){
        ...
        else if(d.x < 0){ return false;}//lock to bounds
        else if(d.y < 0){ return false;}
        else if(d.x > this.w * this.ts){ return false;}
        else if(d.y > this.h * this.ts){ return false;}
        else { d.x = nx; d.y = ny; return true; }//OK
    ...
    MoveBlock(d: any, dt: number, block: number){
        ...
        else { d.x = xy[0]; d.y = xy[1];  return true;}//OK
        return false;//no movement allowed
    }
    //---------------------------------------------------------------------------------------------------------------------------------------------
        ...
        if(this.IsBlocked(xy[0], xy[1], this.ts, this.ts, 0)){
            if(this.IsBlocked(xy[0], d.y, this.ts, this.ts, 0)){//Try with Y same and new X
                if(this.IsBlocked(d.x, xy[1], this.ts, this.ts, 0)){//Try with X same new Y
                    d.tx = d.x; d.ty = d.y;//set to current
                }
                else { d.y = xy[1]; return true; }//OK with just X move
            }
            else { d.x = xy[0];  return true; }//OK with just X move
        }
        else { d.x = xy[0]; d.y = xy[1];  return true;}//OK
        return false;//no movement allowed
    }
    ...
    pProjectile(d: any, dt: number){
        ...
        //lifetime
        if(d.ltc > d.lt){d.remove = 1;} d.ltc += dt;
    }
    //---------------------------------------------------------------------------------------------------------------------------------------------
    //---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    Process(dt: number){
        ...
        for (id in this.units) { if (this.units.hasOwnProperty(id)) { this.pUnit(this.units[id], dt); }}
        for (id in this.projectiles) { if (this.projectiles.hasOwnProperty(id)) { this.pProjectile(this.projectiles[id], dt); }}
        for (id in this.resources) { if (this.resources.hasOwnProperty(id)) { this.pResource(this.resources[id], dt); }}
        for (id in this.buildings) { if (this.buildings.hasOwnProperty(id)) { this.pBuilding(this.buildings[id], dt); }}
    ...
    AIRandMoveSide(d: any, s: any){
        ...
        if(s === 1){d.tx = this.ts/2;}
        if(s === 2){d.ty = this.ts/2;}
        if(s === 3){d.ty = (this.h * this.ts) - this.ts/2;}
    }
